...
from .interview_system import FinalInterviewSystem, InterviewSession, QuestionType, QuestionAnswer
...
from .utils import parse_career_years, get_difficulty_level, safe_json_load, extract_question_and_intent
...
class PersonalizedInterviewSession(InterviewSession):
    ...
    def _create_personalized_plan(self) -> List[Dict[str, Any]]:
        ...
        base_plan = [
            {"type": QuestionType.INTRO, "fixed": True, "personalized": False},
            {"type": QuestionType.MOTIVATION, "fixed": True, "personalized": False},
        ]
...
class PersonalizedInterviewSystem(FinalInterviewSystem):
    ...
    def __init__(self, api_key: str = None, companies_data_path: str = "data/companies_data.json"):
    ...
    def _load_fixed_questions(self) -> Dict[str, List[Dict]]:
        ...
        return safe_json_load("data/fixed_questions.json", default_structure)
    ...
    def _generate_personalized_question(self, session: PersonalizedInterviewSession, 
        ...
        elif question_type == QuestionType.MOTIVATION and not question_plan.get("personalized", False):
            return (
        ...
        try:
            ...
            return extract_question_and_intent(result)
        ...
        for attempt in range(max_attempts):
            ...
            if isinstance(session, PersonalizedInterviewSession) and question_plan.get("personalized", False):
                ...
                question_content, question_intent = self._generate_next_question(
                    session, company_data, question_plan["type"], question_plan.get("fixed", False)
                )
            
            # 중복 검사 (PersonalizedInterviewSession만)
            if isinstance(session, PersonalizedInterviewSession):
