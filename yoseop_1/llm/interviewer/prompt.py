    ...
    def _build_expanded_topic_pools(self) -> Dict[str, List[str]]:
        ...
        return {
            'HR': [
                ...
                '윤리_도덕적판단', '장기_커리어비전', '자기성찰_피드백수용', '책임감_주인의식',
                '다양성_포용성', '목표설정_달성의지', '인간관계_신뢰구축'
            ],
            'TECH': [
                '핵심기술_전문성', '아키텍처_설계경험', '성능최적화_튜닝', '코드품질_리팩토링',
...
2. **장기 근무 가능성**: 해당 회사에서 지속적으로 성장하고 기여할 의지
...
2. **기술적 발전 가능성** : 현재는 능력이 부족할지라도 교육을 통해 성장할 수 있는 성장 잠재력
3. **문제 해결 능력**: 복잡한 기술적 문제를 체계적으로 분석하고 해결하는 능력
4. **기술적 성장성**: 새로운 기술을 빠르게 학습하고 적용할 수 있는 능력
5. **실무 적용 경험**: 이론이 아닌 실제 프로젝트에서의 기술 활용 경험""",
...
"""
    ...
    def build_system_prompt_for_follow_up(self) -> str:
        ...
        🎯 **핵심 원칙** 🎯
        - 각 직무(프론트엔드/백엔드/기획/AI/데이터사이언스)의 고유한 전문성 반영
    ...
    ### 📝 질문 생성 가이드라인 ###
    - ****질문 대상 분리****: 사용자의 답변과 춘식이의 답변을 모두 받되, 춘식이관련 질문은 춘식이에게만 사용자 관련 질문은 사용자에게만 확실하게 하세요.
    ...
    **🎯 질문 방향성**
    ...
    def build_db_template_enhancement_prompt(self, db_template: str, user_resume: Dict, 
        ...
        """DB 고정질문을 LLM으로 튜닝/개선하기 위한 고도화된 프롬프트"""
...
### [질문 개선 임무] ###
🚨 **절대 준수**: 원본 DB 질문을 그대로 복사하거나 유사하게 사용하는 것은 완전히 금지됩니다. 반드시 완전히 새로운 질문으로 재창조해야 합니다.

당신은 {interviewer_role} 면접관으로서, 위의 DB 참조질문을 다음과 같이 **완전히 재작성**해야 합니다:

1. **완전한 재구성**: DB 템플릿의 문장, 표현, 구조를 절대 그대로 사용하지 말고 완전히 다른 방식으로 새롭게 작성
2. **회사별 맞춤화**: 회사명, 기술스택, 문화, 가치관을 구체적으로 언급하여 이 회사만의 질문으로 변경
3. **개인화**: 지원자의 이름과 직군을 자연스럽게 포함하여 개인 맞춤형 질문으로 변경
4. **면접관 캐릭터**: {interviewer_role} 면접관의 말투와 관점을 살려서 실제 면접관이 말하는 것처럼 작성
5. **평가 의도만 유지**: 원본의 표현은 버리고 평가하려는 의도와 핵심 역량만 동일하게 유지

### [개선 원칙] ###
- ❌ **금지**: "어떤", "어떻게", "무엇을" 등의 DB 템플릿 특유의 딱딱한 표현 사용 금지
- ✅ **필수**: 회사 상황과 지원자 맥락을 구체적으로 반영한 생생한 질문
- ✅ **필수**: {interviewer_role} 면접관다운 전문적이면서도 자연스러운 대화체
- ✅ **필수**: 실제 면접 현장에서 자연스럽게 나올 수 있는 형태
